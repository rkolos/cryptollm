...
async function createImmediateTriggers() {
  ...
  const watchlist = (
    ...
    'BTC/USDT,ETH/USDT,SOL/USDT,BNB/USDT,XRP/USDT,ADA/USDT,DOGE/USDT,AVAX/USDT,DOT/USDT,LINK/USDT,UNI/USDT,ATOM/USDT'
  )
    .split(',')
    .map((p) => p.trim());
  ...
  for (let i = 0; i < watchlist.length; i++) {
    ...
    try {
      await pool.query(
        ...
        [
          pair,
          'Immediate trigger - manual activation after position close',
      ...
      console.log(
        `✅ ${pair}: триггер на ${triggerTime} (через ${Math.round((immediateTimeout - Date.now()) / 1000)} сек)`,
      );
    } catch (error) {
