...
| `trigger_conditions` | `Array<TriggerCondition>` | **Да** | Массив новых триггеров. **Заменяет** все старые триггеры для этой пары. **НЕ МОЖЕТ быть пустым массивом `[]`** - минимально должен содержать хотя бы один триггер. Если ты не хочешь отслеживать пару активно, установи timeout триггер на длительный период (например, 60+ минут). |
