...
Создать `SlowCycleService` (Singleton) — главный `setInterval` "пульс" приложения. Этот сервис отвечает за периодические задачи: обновление кэша, плановую сверку состояния, проверку `timeout/indicator` триггеров и выполнение аварийной проверки `Stop-Loss Janitor` (Задача 5.2.1).
