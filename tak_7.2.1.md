...
**Эпик:** 7. 👷 "Исполнитель" (Worker Service) **Задача:** 7.1 Диспетчер "Исполнителя" (WorkerService Dispatcher) **Включает:** 7.1.3 (Публикация) и **7.2 (Реализация `OPEN (Market)`)** **Архитектор:** Gemini **Дата:** 29.10.2025
...
- **`(Новое) pg-protocol/dist/messages`:** (Зависимость) Для `DatabaseError` (проверка `constraint violation`).
...
Этот сервис будет содержать "скелет" диспетчера и заглушки для обработчиков (7.2-7.5).
    ...
    // src/services/WorkerService.ts (Изменения 7.2)
    ...
    import { Decimal } from 'decimal.js';
    import { DatabaseError } from 'pg-protocol/dist/messages'; // (Новое в 7.2)
...
15. **\[Stubs\]** (Больше не Заглушка) `handleOpenPosition` вызывает `_handleOpenMarketPosition` или `_handleOpenLimitPosition` в зависимости от `decision.parameters.type`.
